...
ifndef::cloud-hosted[]
The application might take some time to get ready. See the http://localhost:9094/health[^] URL to confirm that the `bff` microservice is up and running.

Once your application is up and running, open your browser and check out your `frontend` service by going to http://localhost:9090[http://localhost:9090]. 
endif::[]
...
```bash
echo http://${USERNAME}-9090.$(echo $TOOL_DOMAIN | sed 's/\.labs\./.proxy./g')
```
endif::[]
