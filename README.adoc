...
clients to subscribe to events via an HTTP `GET` request to `/bff/sse/`. To
...
receives the message that contain the hostname and the average system load. The
...
`OutboundSseEvent` instance. The `OutboundSseEvent` is broadcasted, or sent to
all registered sinks, by invoking the [hotspot=broadcastEvent
...
[hotspot=eventSource file=0]`EventSource` and specifying `/bff/sse`
...
handler function, and each time an event is received, it will be called.
[hotspot=systemLoadHandler file=0]`systemLoadHandler()` will take the event
