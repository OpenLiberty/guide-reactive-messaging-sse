...
:page-description: Learn how to push Server-Sent Events from a reactive messaging application to a frontend application
...
continuous streams of events. Server-Sent Events provide a convenient way to
...
Server-Sent Events is a specification introduced in HTML5 which allows
...
This guide consists of a `frontend` application and 3 instances of the `system`
service. The `system` services periodically publish messages containing their
hostname and current system load. The `frontend` application provides a frontend
JavaScript client and a backend-for-frontend (`BFF`) service. The `BFF` service
receives the messages from the `system` services and pushes the contents as SSE
to the frontend client that consumes the events and displays a table containing
...
file=0]`@Produces(MediaType.SERVER_SENT_EVENTS)` sets the `Content-Type` in the
response header to `text/event-stream`. This indicates that client requests made
to this endpoint are to receive Server-Sent Events. Additionally, the function's
...
file=0]`@Context` injection and the [hotspot=broadcaster file=0]`broadcaster`
...
Kafka that you will configure in the next section. 
...
client to send the event to. If the `broadcaster` has been set, an
[hotspot=createEvent file=0]`OutboundSseEvent` needs to be created. An instance
of the `OutboundSseEvent.Builder` is obtained by using the
[hotspot=newEventBuilder file=0]`Sse.newEventBuilder()` method which will help
create the `OutboundSseEvent`. Methods from the builder can be chained to set
the [hotspot=name file=0]`name`, [hotspot=data file=0]`data`, and
[hotspot=mediaType file=0]`media type`. Then, the [hotspot=build
file=0]`OutboundSseEvent.Builder.build()` method is called to return the
`OutboundSseEvent` instance. 
...
[hotspot=broadcaster file=0]`broadcaster` is not [hotspot=notNull file=0]`null`.
If it is not null, then there is at least one subscriber, otherwise there is no
client to send the event to. If the `broadcaster` has been set, the
`OutboundSseEvent` is created using the hotspot=newEventBuilder
file=0]`Sse.newEventBuilder()` method and the [hotspot=name file=0]`name` of the
event and the [hotspot=data file=0]`data` it contains, and the
[hotspot=mediaType file=0]`media type` are set. The `OutboundSseEvent` is then
...
A complete `system` service has been provided to you in the `start/system`
...
// =================================================================================================
// Configuring the frontend client to subscribe to and consume events
// =================================================================================================
...
[hotspot=eventSource file=0]`EventSource` and specifying the `/bff/sse`
endpoint in the parameter. The [hotspot=eventSource file=0]`EventSource` will
make a `GET` request to this endpoint with a request header of `Accept:
...
called to add an event listener. This event listener will listen for events with
...
handler function, and each time an event is received, this function will be called.
...
object. The contents of this object, the system's hostname and load, are used to
update the table. If a system is already present in the table, the load is
...
also creates containers for Kafka, Zookeeper, the `frontend` application, and 3
...
updating. If you can see the loads updating, you know that your `BFF` service
is successfully receiving messages and broadcasting them as SSE to the frontend client
to be displayed.
...
You have just developed an application using MicroProfile Reactive Messaging, Open Liberty and Kafka.
