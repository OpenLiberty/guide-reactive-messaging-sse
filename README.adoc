...
To consume the events sent from the server, call the [hotspot=consumeNamedEvent file=0]`EventSource#addEventListener` method to add an event listener. This event listener will listen for events with the name of [hotspot=orderEvent file=0]`order` as specified when creating the [hotspot=name file=1]`OutboundSseEvent`. When the event is received, a [hotspot=eventHandler file=0]`function` is run to handle the event. The [hotspot=eventHandler file=0]`function` will be passed an [hotspot=eventObject file=0]`event` object and [hotspot=parse file=0]`parses` the event's data property, which is an [hotspot=data file=1]`Order`, from a JSON string to a JavaScript object. The [hotspot=callUpdateTable file=0]`updateTable()` function is [hotspot=callUpdateTable file=0]`called` to update the table with the JavaScript object.
