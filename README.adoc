...
hostname and current system load. The `frontend` application has a front-end
...
to this endpoint are to receive Server-Sent Events. Additionally, the methods's
...
file=0]`broadcastData()` method. First, it checks if the
...
file=0]`system.load` in the Kafka message broker. The
