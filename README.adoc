...
The [hotspot=subscribeToOrders file=1]`subscribeToOrders()` function allows clients to subscribe to events via an HTTP request to `GET /api/sse/`. The `/api` is from the [hotspot=path1 file=2]`@ApplicationPath` annotation at the `OpenLibertyCafeApplication` class, the `/sse` is from the [hotspot=path2 file=1]`@Path` annotation at the `OpenLibertyCafeSSEResource` class, and the `/` is the [hotspot=path3 file=1]`@Path` annotation at the `subscribeToOrders()` function. To indicate client requests to receive Server-Sent Events, this endpoint is annotated with [hotspot=sseMimeType file=1]`@Produces(MediaType.SERVER_SENT_EVENTS)` which sets the `Content-Type` in the response header to `text/event-stream`. Additionally, the function's parameters takes in an [hotspot=sseEventSinkParam file=1]`SseEventSink` and an [hotspot=sseParam file=1]`Sse` which are injected using the `@Context` annotation. Firstly, the function checks if the [hotspot=sse file=1]`sse` and [hotspot=broadcaster file=1]`broadcaster` instance variables have been assigned. If not, they will be assigned. This check and assignment is done using the [hotspot=sseAndBroadcaster file=1]`Optional` class with the [hotspot=sse file=1]`sse` being obtained from the [hotspot=sseParam file=1]`@Content` injection and the [hotspot=broadcaster file=1]`broadcaster` being obtained using [hotspot=getBroadcaster file=1]`Sse#newBroadcaster` method. Then, the [hotspot=sseEventSinkParam file=1]`SseEventSink` is [hotspot=registerSink file=1]`registered` to the [hotspot=broadcaster file=1]`SseBroadcaster` to subscribe to events.
...
The [hotspot=reactiveMessagingStream file=1]`receiveOrder()` function subscribes to the [hotspot=incoming file=1]`order` incoming stream which was configured to read from the [hotspot=topic file=0]`statusTopic` topic in Kafka. This function is called whenever one of the other microservices produces a new message to the [hotspot=topic file=0]`statusTopic` topic. When the message is received, which is an Order object, it creates the event and broadcasts it to all subscribers.
