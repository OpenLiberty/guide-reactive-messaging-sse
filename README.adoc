...
continuous streams of events. Server-Sent Events provide a convient way to
...
The application in this guide consists of a `frontend` application and 3
instances of the `system` service. The `system` services periodically publish
messages containing the their hostname and current system load. The `frontend`
application provides a backend-for-frontend (BFF) service that receives these
messages and pushes the contents as SSE to a frontend JavaScript client that
consumes the events and displays a table containing each system's hostname, and
it's periodically updating load. The following diagram depicts the
application that is used in this guide:
...
is successfully receiving messages and broadcasting them as SSE to the JavaScript
