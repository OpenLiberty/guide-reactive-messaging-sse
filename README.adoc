...
containing the their hostname and current system load. The `frontend`
service receives these messages and pushes the contents as SSE to a JavaScript client
that consumes the events and displays a table containing each system's hostname,
and it's periodically updating load. The following diagram depicts the
application that is used in this guide:

image::SSE_Diagram.png[JWT frontend and system services, width=490, height=654, align="center"]
...
clients to subscribe to events via an HTTP `GET` request to `/frontend/sse/`. To
...
the method retrieves the message by connecting to the [hotspot=systemLoadChannel file=1]`systemLoad` channel in
...
The `frontend` service uses an incoming connector to receive messages through
