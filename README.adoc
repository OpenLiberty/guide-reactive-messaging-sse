...
An example of an application that could benefit from using SSE would be a
real-time stock quotes provider. SSE would be an effective way to push price
updates from the server to the browser as soon as the server receives them. In
this circumstance, a Web Socket is unnecessary because data is only travelling one
way, and polling the server using HTTP requests wouldn't provide real-time
updates.
...
In this guide, you will set up the `bff` service by creating an endpoint for
clients to use to subscribe to events. You will also enable the service to read from the
reactive messaging channel and push the contents to subscribers via SSE. After
that, you will configure the Kafka connectors to allow the `bff` service to receive
messages from the `system` services. Finally, you will configure the front-end
client to subscribe to these events, consume them, and display them in the UI.
...
You will build and run the services and the front-end application in Docker containers. You can learn more about containerizing services with Docker in the https://openliberty.io/guides/containerize.html[Containerizing microservices^] guide.
...
pushed from the server. Later in this guide, you will have the front-end client use this
endpoint to subscribe to the events pushed from the `bff` service. 
...
You have now set up an endpoint in the `bff` service that the front-end client
will use to subscribe to events. You have also enabled the service to read from
the reactive messaging channel and broadcast the information as
events to subscribers via SSE.
...
To build the application, navigate to the `start` directory and run the Maven `install` and `package` goals from the command line:
...
Next, use the provided `startContainers.sh` script to start the application in Docker containers. The
...
modern web browsers support Server-Sent Events. The exception is
