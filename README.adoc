...
In this section, you will create a frontend application which will connect to the `OpenLibertyCafe` microservice through the [hotspot=subscribeToOrders file=0]`SSE REST API` and consumes the events when they are pushed. Most of the application has been created except for the JavaScript file which contains the application logic. The frontend UI contains a form to submit new orders which contains a textfield for the table id, a textarea for food orders, and a textarea for beverage orders. Use comma separated values to submit multiple orders in one textarea (i.e, Type `burger, pizza` in the food orders textarea if you wanted to order a burger and a pizza). When the orders are received from the `OpenLibertyCafe` microservice, they are displayed in a table which has columns for the order id, table id, order type, item, and status. There is an additional column with a button to complete an order which appears if the order's status is `READY`.
...
The frontend application subscribes to the SSE by creating a new instance of [hotspot=eventSource file=1]`EventSource` and specifying `/api/sse` endpoint in the parameter. The [hotspot=eventSource file=1]`EventSource` will send a GET request to the `/api/sse` endpoint created in the previous section with a header of `Accept: text/event-stream` to connect to the server.

=== Consuming the Server-Sent Events

To consume the events sent from the server, call the [hotspot=consumeNamedEvent file=1]`EventSource#addEventListener` method to add an event listener. This event listener will listen for events with the name of [hotspot=orderEvent file=0]`order` as specified when creating the [hotspot=name file=0]`OutboundSseEvent`. When the event is received, a [hotspot=eventHandler file=1]`function` is run to handle the event. The [hotspot=eventHandler file=1]`function` will be passed an [hotspot=eventObject file=1]`event` object and [hotspot=parse file=1]`parses` the event's data property, which is an [hotspot=data file=0]`Order`, from a JSON string to a JavaScript object. The [hotspot=callUpdateTable file=1]`updateTable()` function is [hotspot=callUpdateTable file=1]`called` to update the table with the JavaScript object.
