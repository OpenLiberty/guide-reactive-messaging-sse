...
:page-seo-description: A getting started tutorial with examples on how to stream updates as Server-Sent Events (SSE) from a MicroProfile Reactive Messaging Java application to a front-end client.
...
This guide consists of the `frontend` service, the `bff` (backend for frontend)
service, and three instances of the `system` service. The `system` services
periodically publish messages containing their hostname and current system load
and the `frontend` service consists of a front-end JavaScript client. The `bff`
service receives the messages from the `system` services and pushes the contents
as events to the client in the `frontend` service that consumes them and
displays a table that contains each system's hostname and its periodically
updating load. The following diagram depicts the application that is used in
this guide:

image::SSE_Diagram.png[SSE Diagram, align="center"]

In this guide, you will set up the `bff` service by creating an endpoint for
clients to use to subscribe to events. You will also enable the service to read
...
pushed from the server. Later in this guide, you will have the client in  the
`frontend` service use this endpoint to subscribe to the events pushed from the
`bff` service.
...
You have now set up an endpoint in the `bff` service that the client in the
`frontend` service will use to subscribe to events. You have also enabled the
...
In this section, you will configure the client in the `frontend` serivce to subscribe to events
...
loads. This subscribes the client to the SSE by creating a new instance of the
...
Because this request is coming from `localhost:9080` and being made to
`localhost:9084` it's considered to be cross-origin and must follow the Cross-Origin
Resource Sharing (CORS) specification in order to not be blocked by the browser. This is
done by setting the `withCredentials` configuration element to true in the
parameters of the [hotspot=eventSource file=0]`EventSource` interface. CORS has
already been enabled for you in the `bff` service. You can learn more about CORS
by checking out the https://openliberty.io/guides/cors.html[CORS guide].
