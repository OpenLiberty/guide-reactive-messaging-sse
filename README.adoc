...
containing the thier hostname and current system load. The `frontend`
...
the [hotspot=systemLoadChannel file=0]`systemLoad` channel. The messages are then
...
required by the https://kafka.apache.org/documentation/#consumerconfigs[Apache Kafka Consumer
...
file=0]`SseBroadcaster` to subscribe to events.
...
load contained in that message are broadcasted in an event to all subsribers. 
...
[hotspot=systemLoadHandler file=0]`systemLoadHandler()` function is
[hotspot=setHandler file=0]`set` as the handler function, and each time an event
is received, it will be called. [hotspot=systemLoadHandler
file=0]`systemLoadHandler()` will take the event object and [hotspot=parse
file=0]`parse` the event's data property from a JSON string into a JavaScript
