...
Broadcasting events is handled in the [hotspot=reactiveMessagingStream file=1]`receiveOrder()` function. Firstly, it gets the value of the [hotspot=broadcaster file=1]`broadcaster` wrapped in an [hotspot=getBroadcasterOptional file=1]`Optional`. Then, it checks if the broadcaster is [hotspot=isBroadcasterPresent file=1]`present`, meaning that the [hotspot=broadcaster file=1]`broadcaster` is not null. If it is not null, then there is at least one subscriber, otherwise there is no client to send the event to. If the [hotspot=broadcaster file=1]`broadcaster` has been set, an [hotspot=createEvent file=1]`OutboundSseEvent` needs to be created. An instance of the `OutboundSseEvent.Builder` is obtained by using the [hotspot=newEventBuilder file=1]`Sse#newEventBuilder` method which will help create the [hotspot=createEvent file=1]`OutboundSseEvent`. Methods from the builder can be chained to set the [hotspot=id file=1]`id`, [hotspot=data file=1]`data`, [hotspot=name file=1]`name`, [hotspot=delay file=1]`reconnect delay`, [hotspot=comment file=1]`comment`, and [hotspot=mediaType file=1]`media type`. Then, the [hotspot=build file=1]`OutboundSseEvent.Builder#build` method is called to return the [hotspot=createEvent file=1]`OutboundSseEvent` instance. The [hotspot=createEvent file=1]`OutboundSseEvent` is broadcasted, or sent to all registered sinks, by returning the [hotspot=broadcaster file=1]`broadcaster` from the [hotspot=optionalGet file=1]`Optional` and invoking the [hotspot=broadcastEvent file=1]`SseBroadcaster#broadcast` method.
